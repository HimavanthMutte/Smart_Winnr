app.controller('documentController', ['$scope', '$location', '$routeParams', 'documentService', function ($scope, $location, $routeParams, documentService) {
    ...
    $scope.uploadNewVersion = function () {
        if (!$scope.newVersionFile) {
            $scope.error = 'Please select a file for the new version';
            return;
        }
        ...
        var formData = new FormData();
        formData.append('document', $scope.newVersionFile);
        formData.append('changelog', $scope.versionChangelog || '');

        documentService.uploadNewVersion($scope.document._id, formData)
            .then(function (response) {
                $scope.document = response.data;
                $scope.success = 'New version uploaded successfully!';
                $scope.newVersionFile = null;
                $scope.versionChangelog = '';
                $scope.showVersionModal = false;
                // Update file input if exists
                var input = document.getElementById('newVersionInput');
                if (input) input.value = '';
            })
            .catch(function (error) {
                $scope.error = error.data.message || 'Failed to upload new version';
            })
            .finally(function () {
                $scope.loading = false;
            });
    ...
    $scope.restoreVersion = function (version) {
        if (confirm('Are you sure you want to restore version ' + version.version + '? This will create a new version of the document.')) {
            $scope.loading = true;
            $scope.error = '';

            documentService.restoreVersion($scope.document._id, version.version)
                .then(function (response) {
                    $scope.document = response.data;
                    $scope.success = 'Version ' + version.version + ' restored successfully!';
                })
                .catch(function (error) {
                    $scope.error = error.data.message || 'Failed to restore version';
                })
                .finally(function () {
                    $scope.loading = false;
                });
    ...
    $scope.showVersionDialog = function () {
        $scope.newVersionFile = null;
        $scope.versionChangelog = '';
        $scope.showVersionModal = true;
    };

    $scope.hideVersionDialog = function () {
        $scope.showVersionModal = false;
    };

    $scope.setNewVersionFile = function (file) {
        $scope.newVersionFile = file;
        $scope.$apply();
    };
